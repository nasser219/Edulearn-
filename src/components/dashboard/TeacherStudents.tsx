...
export const TeacherStudents = ({ selectedCourseId, onBack }: { selectedCourseId?: string | null, onBack?: () => void }) => {
  ...
  useEffect(() => {
    ...
    const fetchStudentData = async () => {
      ...
      try {
        ...
        setStudentEnrollment(enrollmentSnap.docs[0] ? { id: enrollmentSnap.docs[0].id, ...enrollmentSnap.docs[0].data() } : null);
      } catch (error) {
  ...
  const handleRemoveStudentFromCourse = async () => {
    if (!studentEnrollment?.id || !selectedStudent) return;
    
    const confirmRemove = window.confirm(`هل أنت متأكد من رغبتك في إخراج الطالب (${selectedStudent.fullName}) من كورس (${selectedStudent.courseTitle})؟ \n\nسيتم قفل الكورس عنده فوراً.`);
    
    if (!confirmRemove) return;

    try {
      await deleteDoc(doc(db, 'enrollments', studentEnrollment.id));
      alert('تم إخراج الطالب من الكورس بنجاح. ✅');
      setSelectedStudent(null);
    } catch (error) {
      console.error("Error removing student from course:", error);
      alert('حدث خطأ أثناء محاولة إخراج الطالب.');
    }
  };
            ...
            userSnap.docs.forEach(uDoc => {
              ...
              studentMatches.forEach(match => {
                studentsData.push({
                  ...
                  courseTitle: courseMap.get(match.courseId) || 'دورة غير معروفة'
                });
              });
            });
          }
  ...
  const filteredStudents = students.filter((s: Student) => {
    ...
    const studentCourseId = teacherCourses.find(c => c.title === s.courseTitle)?.id;
    ...
    <div className="space-y-8 animate-in fade-in duration-700" dir="rtl">
                  ...
                  <select 
                   ...
                   >
                     ...
                     {(selectedStage === 'ALL' 
                       ...
                       : (GRADES[selectedStage as keyof typeof GRADES] || [])
                     ).map((g: any) => (
              ...
              <table className="w-full text-right min-w-[800px]">
                ...
                <tbody className="divide-y divide-slate-50">
                  {loading ? (
                    ...
                    <tr><td colSpan={6} className="p-20 text-center text-slate-400 font-bold">لا يوجد طلاب مشتركين في دوراتك حالياً. 🔍</td></tr>
                  ) : (
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
           ...
           <Card className="relative w-full max-w-4xl bg-white rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col focus:outline-none">
                 ...
                 <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                     <Button 
                       ...
                       className="h-12 px-6 rounded-xl font-black text-sm transition-all bg-red-50 text-red-600 border-red-100 hover:bg-red-600 hover:text-white"
                       title="إخراج من الكورس"
                     >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                           ...
                           <div className="space-y-2">
                              ...
                              >
                                ...
                                {((isEditing ? editData.stage : selectedStudent.stage) && GRADES[isEditing ? editData.stage : (selectedStudent.stage as keyof typeof GRADES)])?.map((g: any) => (
                                  <option key={g.id} value={g.id}>{g.label}</option>
                                ))}
                              </select>
                           </div>
